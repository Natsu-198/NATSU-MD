...
async (conn, mek, m, { q, reply }) => {
    try {
        ...
        if (!q || !/^\d{8,15}$/.test(q)) {
            return await reply("❌ Invalid phone number. Example: `.pair 50943449848`");
        }

        // Fetch pairing code from API
        const response = await fetch(`https://Natsu-tech-web.onrender.com/pair/code?number=${q}`);
